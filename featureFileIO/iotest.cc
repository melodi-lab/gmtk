...
main(int argc, const char **argv) {
  ...
  printf("num output files: %i\n",num_outfiles);
  
  in_str = new DataInStream(num_files,
			    (const char **)in_names,
			    (const char **)icrange,
			    (const char **)idrange,
			    in_nfloats,
  ...
  for (int o = 0; o < num_outfiles; o++) {
    ...
    ocrng[o] = new BP_Range(ocrange[o],0,in_str->getNumFloats());
    odrng[o] = new BP_Range(odrange[o],0,in_str->getNumInts());
    
    out_floats[o] = ocrng[o]->length();	
    out_ints[o] = odrng[o]->length();
    
    char *tmp = out_formats[o];
    if (strcmp(tmp,"htk") == 0)
      oformats[o] = HTK;
   else if (strcmp(tmp,"binary") == 0)
      oformats[o] = RAWBIN;
  ...
  printf("Command line summary for output files:\n");
  for (int g = 0; g < num_outfiles; g++) {
    printf("file %i: nfloats: %i nints: %i format: %i ",g,
	   out_floats[g],out_ints[g],oformats[g]);
  }

  if (num_outfiles > 0)
    out_str = new DataOutStream(num_outfiles,
				out_floats,
				out_ints,
				oformats,
				NULL,
  ...
  for (size_t f = 0; f < in_str->numSegs() ; f++) {
    ...
    // ObservationMatrix *o = in_str->getObs();
    // for (size_t r = 0; r < num_frames; r++) 
    //  o->printFrame(stdout,r);
    
    if (num_outfiles > 0) {
      in_str->getFileNames(f,out_names);
      out_str->initFiles(out_names,out_ext,num_frames);
      out_str->writeData(f,ocrange,odrange,"all",in_str->getObs());
      out_str->closeFiles(); 
    }
  }
