 ...
 *      calls makeRandom for the mean,covar and dlinks
 *
 * Preconditions:
 ...
 *      calls makeUniform for the mean,covar and dlinks
 *
 * Preconditions:
 *      Object must be allocated.
 *
						    ...
						    bool writeZeros)
{
  ...
  ofile.writeComment("LinMeanCondDiagGaussian %s xAcc len %u  xxAcc len %u  xzAcc len %u  zzAcc len %u  zAcc len %u:"
		     ...
		     trMembers->xAccumulators.len(), trMembers->xxAccumulators.len(), trMembers->xzAccumulators.len(), 
		     trMembers->zzAccumulators.len(), trMembers->zAccumulators.len());
  if (writeZeros) {
    ...
    for (unsigned i=0;i<totalLen;i++) {
      ofile.write(trMembers->xAccumulators[0], 0.0,"LMDG zero accums x.");
    }
  } else {
