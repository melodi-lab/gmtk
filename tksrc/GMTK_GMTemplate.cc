...
writeSections(oDataStreamFile& os) {
  ...
  os.write(PCInterface_in_C.size()); os.nl();
  for (unsigned j=0; j < PCInterface_in_C.size(); ++j) {
    os.write(j);
    os.write(PCInterface_in_C[j].size());
    for (set<RV*>::iterator i = PCInterface_in_C[j].begin();
	 i != PCInterface_in_C[j].end(); i++) {
      RV* rv = (*i);
      os.write(rv->name().c_str(),"rv name");
      os.write(rv->frame(),"rv frame");
    }
  ...
  os.write(CEInterface_in_C.size()); os.nl();
  for (unsigned j=0; j < CEInterface_in_C.size(); ++j) {
    os.write(j);
    os.write(CEInterface_in_C[j].size());
    for (set<RV*>::iterator i = CEInterface_in_C[j].begin();
	 i != CEInterface_in_C[j].end(); i++) {
      RV* rv = (*i);
      os.write(rv->name().c_str(),"rv name");
      os.write(rv->frame(),"rv frame");
    }
