					 ...
					 bool writeZeros)
{
  ...
  if (writeZeros) {
    ...
    for (int i=0; i < trMembers->nextMeans.len(); i++) {
      ofile.write(trMembers->nextMeans[0], 0.0, "MissingFeatureScaledDiagGaussian store accums nm + nc.");
    }
    ofile.nl();
    ...
    for (int i=0; i < trMembers->nextDiagCovars.len(); i++) {
      ofile.write(trMembers->nextDiagCovars[0], 0.0, "MissingFeatureScaledDiagGaussian store accums nm + nc.");
    }
    ofile.nl();
    ofile.writeComment("MFSDG elementAccumulatedProbability len %u:  ... elementAccumulatedProbability[i] ...\n",
		       trMembers->elementAccumulatedProbability.len());
    for (int i=0; i < trMembers->elementAccumulatedProbability.len(); i++) {
      ofile.write(trMembers->elementAccumulatedProbability[0].valref(), trMembers->elementAccumulatedProbability[0].valref()*0.0,"El Acc Prob 0.0");
    }
    ofile.nl();
  } else {
    ofile.writeComment("MFSDG MeanVector %s len %u:  ... nextMeans[i] ...\n", mean->name().c_str(), trMembers->nextMeans.len());
