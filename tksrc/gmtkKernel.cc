...
void updateBoth(double lr, double covarLr, double meanLr,  map<string, vector_d>& adagrad, bool use_adagrad) {
    ...
    if(update_DPMF) {
	    ...
	    double acc_val; 
            for(unsigned j=0; j<pmfs.size(); j++) {
                acc_val = next_pmfs[j].unlog() - down_weight_dpmf * next_pmfs_d[j].unlog();
                if(use_adagrad) {
                    double old_val = adagrad[name][j];
                    local_lr = lr / old_val;
                    adagrad[name][j] = sqrt(old_val * old_val + acc_val * acc_val);
...
main(int argc,char*argv[])
{
    ...
    for(unsigned iter=0; iter<max_iter; iter++) {
	    ...
	    // fprintf(stderr, "range: %u %u\n", start, end);
            ...
            updateBoth(lr, covarLr, meanLr, adagrad, use_adagrad);
            //getGmParmsValue();
