...
#undef GMTK_ARGUMENTS_CHECK_ARGS
  ...
  while (!dcdrng_it->at_end()) {
    ...
    if (wordVar && probe.not_essentially_zero())
    {
      ...
      for (int i=0; i<int(myjt.curNodes().size()); i++)
      {
        if (myjt.curNodes()[i]->name()=="wordTransitionA"
          ||myjt.curNodes()[i]->name()=="wordTransitionB"
	  ||myjt.curNodes()[i]->name()=="wordA"
	  ||myjt.curNodes()[i]->name()=="wordB"
	  ||myjt.curNodes()[i]->name()=="time")
        {
	  printf("frame=%d %s=%d\n",
		 myjt.curNodes()[i]->frame());
		 myjt.curNodes()[i]->name().c_str(),
		 RV2DRV(myjt.curNodes()[i])->val,
        }
      }
    }
    (*dcdrng_it)++;
