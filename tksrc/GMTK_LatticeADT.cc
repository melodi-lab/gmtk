...
//            printf("enlarge cpt array size: %u from orig_size: %u\n", delta, node.larger_cpts.size());
        ...
        if(node.larger_cpts[delta] == NULL) {
            cpt_new = true;
            shash_map_iter<unsigned, LatticeADT::LatticeEdgeList> new_cpt;
...
void LatticeADT::dfsOnNode(LatticeADT::LatticeNode & cur_node, unsigned delta, shash_map_iter<unsigned, LatticeADT::LatticeEdgeList> & result) {
...
void LatticeADT::addNodeEdge2Map(shash_map_iter<unsigned, LatticeADT::LatticeEdgeList> & added, unsigned child_id, const LatticeADT::LatticeEdge & edge) {
