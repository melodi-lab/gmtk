...
static void
pfile_ftrcombo(
	    InFtrLabStream_PFile& in1_stream,
	    InFtrLabStream_PFile* pin2_stream,
	    InFtrLabStream_PFile* pin3_stream,
	    InFtrLabStream_PFile* pin4_stream,
...

{
	...
	if (fr1rng.length() != fr2rng.length()) {
	    error("num sel ftr1 ftrs (%d) does not match num sel ftr2 ftrs (%d)\n",
	...
	if (fr1rng.length() != fr3rng.length()) {
	    error("num sel ftr1 ftrs (%d) does not match num sel ftr3 ftrs (%d)\n",
	...
	if (fr1rng.length() != fr4rng.length()) {
	    error("num sel ftr1 ftrs (%d) does not match num sel ftr4 ftrs (%d)\n",
    ...
    UInt32* lab3_buf = new UInt32[buf_size * n3_labs];
    UInt32* lab4_buf = new UInt32[buf_size * n4_labs];
    UInt32* lab_buf_p;

    UInt32* olab_buf = new UInt32[buf_size * lrrng.length() ];
    UInt32* olab_buf_p;
	...
	if ((*srit1) % 100 == 0)
	  printf("Processing sentence %d\n",(*srit1));
        ...
        if (n1_frames == SIZET_BAD /* || n2_frames == SIZET_BAD */)
        {
            fprintf(stderr,
	...
	Range prrng(pr_str,0,n1_frames);
        ...
        if (n1_frames > buf_size)
        {
            ...
            olab_buf = new UInt32[buf_size * lrrng.length() ];
            lab1_buf = new UInt32[buf_size * n1_labs];
            lab2_buf = new UInt32[buf_size * n2_labs];
            lab3_buf = new UInt32[buf_size * n3_labs];
            lab4_buf = new UInt32[buf_size * n4_labs];

        }

        const SegID seg1_id = in1_stream.set_pos((*srit1), 0);
        if (seg1_id == SEGID_BAD)
        {
            fprintf(stderr,
	...
	if (pin2_stream) {
	    const SegID seg2_id = pin2_stream->set_pos((*srit2), 0);
	    if (seg2_id == SEGID_BAD)
	    {
		fprintf(stderr,
		       "%s: Couldn't seek to start of sentence %lu "
	...
	if (pin3_stream) {
	    const SegID seg3_id = pin3_stream->set_pos((*srit3), 0);
	    if (seg3_id == SEGID_BAD)
	    {
		fprintf(stderr,
		       "%s: Couldn't seek to start of sentence %lu "
		       "in 3rd input pfile.",
	...
	if (pin4_stream) {
	    const SegID seg4_id = pin4_stream->set_pos((*srit4), 0);
	    if (seg4_id == SEGID_BAD)
	    {
		fprintf(stderr,
		       "%s: Couldn't seek to start of sentence %lu "
		       "in 4th input pfile.",
	...
	out_stream.doneseg((SegID) seg1_id);
