...
static void
pfile_klt(InFtrLabStream_PFile* in_streamp,
	  ...
	  const bool ascii)
{
    ...
    const size_t n_labs = in_streamp->num_labs();
    ...
    if (in_st_fp == NULL) {
	  ...
	  const SegID seg_id = in_streamp->set_pos((*srit), 0);
	...
	// Write output.
	out_streamp->write_ftrslabs(n_frames, oftr_buf, lab_buf);
	out_streamp->doneseg((SegID) seg_id);
