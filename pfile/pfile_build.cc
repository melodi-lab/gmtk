/*
    ...
    Modified to work more generally by 
...
"-bswap          do byteswapping on output\n"
...
pfile_build(SentIdStream_File& sent_id_stream,
		...
		OutFtrLabStream_PFile& output_stream,
		int nfeats,
		char *suffix,
		bool quiet)
{
	...
	if (feat_len % (sizeof(float)*nfeats))
	  error("Length of %s%s is not k*sizeof(float)*nfeats(=%d).",
        ...
        UInt32* label = new UInt32[n_frames];
	...
	if (label_vals_stream_p.nullFile()) {
	  ...
	  output_stream.write_ftrslabs(n_frames, feature, label);
        ...
        output_stream.doneseg((SegID) seg_id++);
...
main(int argc, const char *argv[])
{
    ...
    OutFtrLabStream_PFile* output_stream_p
