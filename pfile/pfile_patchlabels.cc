...
"-iswap       byte-swap input feature file\n"
...
public:
    LabelValsStream_InStr(InLabStream* a_instr) { 
	instr = a_instr;
	instr->nextseg();
    }
...
public:
	...
	LabelValsStream_InStr(new InFtrLabStream_PFile(0 /*debug_level*/, 
							 "plabels:labf", 
							 fp, 0)) { };
...
public:
	...
	LabelValsStream_InStr(new InLabStream_ILab(0 /*debug_level*/, 
						      "plabels:labf", 
						      fp, 0)) { };
...
public:
	...
	LabelValsStream_InStr(new InFtrLabStream_PreFile(0 /*debug_level*/, 
							 "plabels:labf", 
							 fp, width, 0)) { };
...
public:
	...
	LabelValsStream_InStr(new InFtrLabStream_LNA8(0 /*debug_level*/, 
							 "plabels:labf", 
							 fp, width, 0)) { };
...
plabels(LabelValsStream& label_vals_stream_p,
	InFtrLabStream& in_stream,
	OutFtrLabStream& out_stream)
{
    ...
    for (cur_sent = 0; cur_sent < end_sentence ; cur_sent++) {
        ...
        const SegID seg_id = in_stream.set_pos(cur_sent, 0);
        if (seg_id == SEGID_BAD)
        {
            fprintf(stderr,
...
main(int argc, const char *argv[])
{
	...
	out_stream_p
        = new OutFtrLabStream_LNA8(debug_level,
				       "plabels:opf",
                                       opfile_fp,
                                       in_stream_p->num_ftrs());
    } else { /* could have more, but what for? */
