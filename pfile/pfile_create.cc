...
public:
  ...
  FrameStr(FILE*,const bool,size_t,size_t,bool);
  ~FrameStr();
...
FrameStr::FrameStr(FILE *in_fp_a,
  ...
  : n_ftrs(n_ftrs_a),n_labs(n_labs_a),
    binary(binary_a),in_fp(in_fp_a),bswap(swap)
{
  ftrs = new float[n_ftrs];
  labs = new UInt32[n_labs];
  sent_no = MAX_SIZE_T;
  frame_no = MAX_SIZE_T;
}
			     ...
			     const size_t n_labs)
{
  if (buf_size+1 > buf_array_length) {
    ...
    UInt32* lab_tmp = new UInt32[buf_array_length * n_labs];
    ::memcpy(ftr_tmp,ftr_buf,sizeof(float)*buf_size*n_ftrs);
    ::memcpy(lab_tmp,lab_buf,sizeof(UInt32)*buf_size*n_labs);
    delete ftr_buf;
    delete lab_buf;
    ftr_buf = ftr_tmp;
    lab_buf = lab_tmp;
		    ...
		    size_t &buf_array_length)
{
  ...
  while(framestr.nextFrame()) {
    ...
    if (framestr.frame_no != n_frames) {
      error("ERROR: Frame numbers must be in sequence.Current sent = %d, %li %li",cur_sent_no,framestr.frame_no,n_frames);
    }
             ...
             const bool iswap)
{
    ...
    UInt32* lab_buf = new UInt32[buf_array_length * n_labs];
    size_t n_frames=0;
    size_t sent_no=0;

    size_t total_frames = 0;
    FrameStr framestr(in_fp,binary,n_ftrs,n_labs,iswap);
