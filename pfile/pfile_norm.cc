...
static void
pfile_norm(InFtrLabStream_PFile& in_stream,
	   ...
	   int *seg_markers)
{
	...
	if (debug > 0) 
        ...
        if (n_frames > buf_size)
	  {
        ...
        const SegID seg_id = in_stream.set_pos((*srit2), 0);
	...
	for (Range::iterator prit=prrng.begin();
	  ...
	  UInt32* lab_buf_p = lab_buf + (*prit)*n_labs;
	  ftr_means_p = ftr_means;
	  ftr_stds_p = ftr_stds;
	  for (Range::iterator frit=frrng.begin();
	       !frit.at_end(); ++frit) {
...
main(int argc, const char *argv[])
{
    ...
    UInt32 segment_length = INT_MAX; // treat all sentences as one big segment
	...
	else if (strcmp(argp, "-sl")==0)
	{
		 ...
		 segment_length=(UInt32)(i?i:INT_MAX);
		 argc--;
	     }
             else
    ...
    InFtrLabStream_PFile* in_streamp
        = new InFtrLabStream_PFile(debug, "", in_fp, 1,iswap);

    UInt32 pfile_size= in_streamp->num_segs();
    if (pfile_size==SIZET_BAD) 
	...
	if ((segCount+(UInt32)segLen)<=pfile_size) {
	...
	error(errstr);
      ...
      for(UInt32 i=0;i<pfile_size;i+=segment_length) {
	for(UInt32 j=0;j<segment_length && j+i<pfile_size; j++) 
    ...
    OutFtrLabStream_PFile* out_streamp
        = new OutFtrLabStream_PFile(debug,
				       "",
                                       out_fp,
				       fr_rng->length(),
				       lr_rng->length(),
