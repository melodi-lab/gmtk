...
usage(const char* message = 0)
{
    ...
    fprintf(stderr,
	    ...
	    "-i2 <file-name> second input pfile\n"
            "-iswap1         byte-swap file 1 \n"
            "-iswap2         byte-swap file 2 \n"
	    ...
	    "-tol% float     make tolarance a percentage [0-100]\n"
...
static void
pfile_diff(InFtrLabStream_PFile& in1_stream,
	   InFtrLabStream_PFile& in2_stream,
	   ...
	   bool stop)
{
	...
	if (pr1rng.length() != pr2rng.length()) {
	  error("Num frames of per-sentence ranges in sentences %d of file1 and %d of file2 different. pf1 = %d, pf2 = %d\n", 
        ...
        const SegID seg2_id = in2_stream.set_pos((*srit2), 0);
	...
	for (; !prit1.at_end() ; ++prit1,++prit2) {
	  ...
	  for (;!frit1.at_end(); ++frit1,++frit2) {
		...
		if (stop)
	      ...
	      float dem = (fabs(ftr1_buf_p[*frit1]) + fabs(ftr2_buf_p[*frit2]));
	      if (dem == 0.0) {
...
main(int argc, const char *argv[])
{
    ...
    InFtrLabStream_PFile* in2_streamp
        = new InFtrLabStream_PFile(debug_level, "", in2_fp, 1,iswap2);
