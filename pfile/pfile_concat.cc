...
usage(const char* message = 0)
{
    ...
    fprintf(stderr,
	    ...
	    "-lr <range>     label range\n"
            "-iswap          do byteswapping on (all) input files\n"
            "-oswap          do byteswapping on output\n"
...

{
    ...
    OutFtrLabStream_PFile* out_stream
        = new OutFtrLabStream_PFile(debug_level, "", out_fp,
    ...
    UInt32 *olab_buf = new UInt32[buf_size * lr_rng->length() ];
    UInt32 *lab_buf_p, *olab_buf_p;
	...
	for (Range::iterator srit=sr_rng.begin();!srit.at_end();srit++) {
	    ...
	    if (n_frames > buf_size)
		{
		    ...
		    olab_buf = new UInt32[buf_size * lr_rng->length() ];
		}
	    ...
	    out_stream->doneseg((SegID) out_seg_id++);
	}
