/*
    ...
    This program deltass an arbitrary subset set of features from each
...
static void
pfile_deltas(InFtrLabStream_PFile& in_stream,
	     ...
	     bool make_ddeltas)
{
    ...
    float *ftr_buf_p;
    float *oftr_buf = new float[buf_size * frrng.length()*(make_ddeltas?3:2)];
    float *oftr_buf_p;
	...
	if ((*srit) % 100 == 0)
	  printf("Processing sentence %d\n",(*srit));
        ...
        if (n_frames > buf_size)
        {
            ...
            oftr_buf = new float[buf_size * frrng.length()*(make_ddeltas?3:2)];
...
main(int argc, const char *argv[])
{
    ...
    InFtrLabStream_PFile* in_streamp
        = new InFtrLabStream_PFile(debug_level, "", in_fp, 1);
    ...
    OutFtrLabStream_PFile* out_streamp
        = new OutFtrLabStream_PFile(debug_level,
				       "",
                                       out_fp,
				       fr_rng->length()*(make_ddeltas?3:2),
				       lr_rng->length(),
                                       1);
    ...
    pfile_deltas(*in_streamp,
		 *out_streamp,
		 *sr_rng,*fr_rng,*lr_rng,pr_str,make_ddeltas);
